...
All post-processing steps were organized in a single python script (scVelo_withClustering-labelling.py). Hereafter we briefly describe the most important steps.
...
That image has several problems. The first one is that, although we see arrows representing trajectories of cells that transition into other cell states, nothing is seen about clusters colors and cell types.
...
Run the above commands in the python console. It will print the path in which the models were stored. It should be something like: '/home/userName/.celltypist/data/models/'. Then use the corresponding model for cell classification:
...
scVelo can also be implemented in R using the library [reticulate](https://rstudio.github.io/reticulate/) to leverage python code. Due to time constrains, it is not explained here. However, if it becomes important to any of you, you can request a tutorial on scVelo implementation in R and I will do it. 
